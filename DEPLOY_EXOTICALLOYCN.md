...
The same ID is forwarded to the downstream webhook in `X-Tongjun-Request-Id` and stored in the webhook JSON record as `request_id`. R15.19 expands the random suffix from 8 to 12 hexadecimal characters (48 bits).
...
## R15.19 RFQ trust-boundary contract
