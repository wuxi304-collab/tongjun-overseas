# Deploy exoticalloycn.com — V34.152 R15.18
...
The same ID is forwarded to the downstream webhook in `X-Tongjun-Request-Id` and stored in the webhook JSON record as `request_id`. R15.18 expands the random suffix from 8 to 12 hexadecimal characters (48 bits).
...
## R15.18 release identity contract
...
The production `/api/health` response exposes the same site/visual release plus the Vercel git commit and sends `X-Tongjun-Release: V34.152 R15.18`.
...
## R15.18 RFQ trust-boundary contract
...
11. `/.well-known/release.json` returns R15.18 production identity.
...
8. Verify `/.well-known/release.json` and `/api/health` expose the same R15.18 source commit.
