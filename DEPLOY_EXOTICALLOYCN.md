# Deploy exoticalloycn.com — V34.152 R15.11
...
## R15.11 release identity contract
...
The production `/api/health` response exposes the same site/visual release plus the Vercel git commit and sends `X-Tongjun-Release: V34.152 R15.11`.
...
8. Verify `/.well-known/release.json` and `/api/health` expose the same R15.11 source commit.
