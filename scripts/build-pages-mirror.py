...
# Pages is a temporary public mirror. Keep internal sales/ops and server-only
# material out of the artifact even when those files exist in the repository.
SKIP_TOP_LEVEL = {
...
def main():
    ...
    leaked = [p for p in OUT.rglob('*') if p.is_file() and p.relative_to(OUT).parts[0] in {'ops', 'api', 'scripts', 'tests'}]
    if leaked:
    ...
    print(f'PASS: R15 Pages mirror built — {html_count} root pages, noindex enforced, internal ops/server paths excluded, {copied} public files copied.')
