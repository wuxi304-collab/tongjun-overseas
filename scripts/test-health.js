...
async function run(){
  ...
  assert.equal(res.payload.site_release,'V34.152 R15.18');
  ...
  // HTTPS without an HMAC secret is still blocked.
  ...
  // Production readiness requires both HTTPS delivery and a signing secret.
  process.env.RFQ_SHARED_SECRET='health-test-secret';
  res=makeRes();
  await handler({method:'GET',headers:{}},res);
  ...
  console.log('PASS: R15.18 production health requires HTTPS RFQ routing + HMAC secret, with release identity, HEAD and method gates validated.');
}
