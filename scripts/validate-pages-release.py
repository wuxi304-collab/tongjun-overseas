...
def main():
    ...
    if not release_path.is_file():
        fail('R15.19 Pages release identity missing')
    try:
        release = json.loads(release_path.read_text(encoding='utf-8'))
    except Exception as exc:
        fail(f'R15.19 Pages release identity is invalid JSON: {exc}')
    expected_release = {
    ...
    for key, expected in expected_release.items():
        if release.get(key) != expected:
            fail(f'R15.19 Pages release identity mismatch for {key}: {release.get(key)!r}')
    if not re.fullmatch(r'[0-9a-f]{40}', str(release.get('source_commit') or '')):
        fail('R15.19 Pages source_commit must be a 40-character git SHA')
    ...
    if release.get('visual_manifest_sha256') != expected_visual_sha:
        fail('R15.19 Pages visual manifest SHA256 mismatch')
    ...
    print(
        'PASS: R15.19 Pages release — 50 root pages, staging noindex, observable release identity, keyboard landmarks/focus CSS gated, '
