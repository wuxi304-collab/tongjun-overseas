...
for(const required of ['api/rfq.js','api/health.js','vercel.json','.vercelignore','robots.txt','sitemap.xml','RELEASE_R15_10.json','VISUAL_MANIFEST_R15_7.json','scripts/write-release-metadata.py','scripts/check-production-readiness.js','scripts/smoke-rfq-production.js']){
...
if(!vi.includes('!RELEASE_R15_10.json')) failures.push('.vercelignore must include R15.10 release descriptor for build identity');
...
for(const marker of ['rfq_route_configured','Cache-Control','RFQ_WEBHOOK_URL','V34.152 R15.10','X-Tongjun-Release','visual_release']) if(!health.includes(marker)) failures.push(`api/health.js missing ${marker}`);
