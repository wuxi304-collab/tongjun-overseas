...
def main():
    ...
    if not release_path.is_file():
        fail('R15.9 production release identity missing')
    try:
        release = json.loads(release_path.read_text(encoding='utf-8'))
    except Exception as exc:
        fail(f'R15.9 production release identity is invalid JSON: {exc}')
    expected_release = {
        'service': 'tongjun-overseas',
        'site_release': 'V34.152 R15.9',
        'source_branch': 'feat/v34.152-r15.9-media-performance',
    ...
    for key, expected in expected_release.items():
        if release.get(key) != expected:
            fail(f'R15.9 production release identity mismatch for {key}: {release.get(key)!r}')
    if not re.fullmatch(r'[0-9a-f]{40}', str(release.get('source_commit') or '')):
        fail('R15.9 production release source_commit must be a 40-character git SHA')
    ...
    if release.get('visual_manifest_sha256') != expected_visual_sha:
        fail('R15.9 production release visual manifest SHA256 mismatch')
    ...
    print(
        ...
        '0 internal leaks, 0 broken assets/routes/anchors, R15.9 release identity and current RFQ runtime active.'
    )
